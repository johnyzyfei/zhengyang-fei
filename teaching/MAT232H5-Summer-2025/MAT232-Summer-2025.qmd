...
Suppose that $T$ is a one-to-one transformation, given by $x = g(u, v)$, $y = h(u,v)$, with nonzero Jacobian that maps the region $S$ in the uv-plane onto the region $R$ in xy-plane. If $f$ is continuous on $R$, then
$$\iint_R f(x, y) \, dA = \iint_S f\big(g(u, v), h(u, v)\big) 
