...
(ii) determine the limits of integration (i.e., $a, b, c,$ and $d$) in $$P\left(Y > \frac{1}{2}\right) = \int_a^b \int_{c}^d f_{X,Y}(x, y) \, dy \, dx$$ and list below
:::


::: {.callout-tip collapse="true"}
### Solution
Refer to this [link](https://www.desmos.com/calculator/yyllpbo6jq). 

**a)** The support is the overlapping region between the red and blue.
$$S = \left\{ (x, y) \in \mathbb{R}^2 \;\middle|\; 0 \leq y \leq x \leq 1 \right\}$$
**b)** For this probability, we look at the overlapping region between the red, blue, and purple. Hence we have $a = 0.5, b = 1, c = 0.5, d = x$.
:::
...
::: {.callout-tip collapse="true"}
### Solution
Refer to this [link](https://www.desmos.com/calculator/pa6sukqq8q).

**a)** The support is the overlapping region between the red and blue.
$$S = \left\{ (x, y) \in \mathbb{R}^2 \;\middle|\; 0 \leq x \leq 1, 0 \leq y \leq 2x \right\}$$
**b)** For this probability, we look at the overlapping region between the red, blue, and green. Hence we have $a = 0.5, b = 1, c = 0, d = 2x$
...
60x^2(1 - x)^3, & 0 < x < 1 \\
0, & \text{otherwise}
\end{cases}$$ Find the pdf of $Y = X^3$.
:::

::: {.callout-tip collapse="true"}
### Solution

We can use the method of transformations directly or the derivative cdf of X first and then use the cdf method. Here, transformation works faster.

First, we look at all possible $Y$ values. Since from $X$, we have $0 < X < 1$, applying the transformation we get $0^3 < Y = X^3 < 1^3 \implies 0 < Y < 1$.

Next, note that the transformation $Y = X^3$ is invertible So $Y = X^3 \implies X = Y^{1/3}$. So we can directly use the transformation formula 
$$f_Y(y) = f_X(y^{1/3}) \left| \frac{d}{dy} y^{1/3} \right|
= 60 y^{2/3} (1 - y^{1/3})^3 \left( \frac{1}{3} y^{-2/3} \right)
= 20 (1 - y^{1/3})^3 \quad \text{for } 0 < y < 1$$
...
**a)** 
Let S =  Smoker, S' = Non Smoker. Then
$$P(S) = \sum_{\text{all ages}} p(S, age) = \frac{25+125+150}{1000} = \frac{300}{1000} = 0.3$$
$$P(S^c) = 1 - P(S) = 0.7$$

Let A = age $\le 18$, B =  age $19-54$, C = age $55+$. Then
$$P(A) = \sum_{\text{all smoking status}} P(A, \text{smoking status}) = \frac{25+260}{1000} = \frac{285}{1000}$$
$$P(B) =  \frac{125+205}{1000} = \frac{330}{1000}$$
$$P(C) =  \frac{150+235}{1000} = \frac{385}{1000}$$
**b)**
We first change the events to random variables. So our table becomes
...
To quantify their relationships, we can calculate their covariance. The covariance is a measure of the linear association between two random variables. 

Recall the formula for Cov$(X, Y) = \mathbb{E}(XY) - \mathbb{E}(X) \mathbb{E}(Y)$. So we need to calculate:

$$\mathbb{E}[X] = \frac{1 \cdot 300 + 0 \cdot 700}{1000} = \frac{300}{1000} = 0.3$$
$$\mathbb{E}[Y] = \frac{1 \cdot 285 + 2 \cdot 330 + 3 \cdot 385}{1000}
= \frac{285 + 660 + 1155}{1000}
= \frac{2100}{1000} = 2.1$$

$$\mathbb{E}[XY] = \frac{1 \cdot 1 \cdot 25 + 1 \cdot 2 \cdot 125 + 1 \cdot 3 \cdot 150 + 0 \cdot 1 \cdot 260 + 0 \cdot 2 \cdot 205 + 0 \cdot 3 \cdot 235}{1000}
= \frac{25 + 250 + 450}{1000}
 = 0.725$$

Putting everything together,
$$\text{Cov}(X, Y) = \mathbb{E}[XY] - \mathbb{E}[X] \mathbb{E}[Y]
= 0.725 - (0.3 \cdot 2.1)
= 0.725 - 0.63 = \boxed{0.095}$$

The positive covariance between smoking status and age group suggests that older individuals are more likely to smoke than younger individuals in this population.

While we know the direction of the relationship (positive), we cannot interpret the magnitude of the covariance. This is because the covariance is not normalized. To determine the strength of the relationship, we need to normalize it by the variance of each variable to find their correlation (to be covered in later courses).
:::
