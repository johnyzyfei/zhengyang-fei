...
If there were no censoring, this would be an easy task as we could directly use the ECDF 
$$\hat{F_n}(t) = \frac{1}{n} \sum_{i=1}^n I(t_i \le t) \implies \hat{S}_n(t) = 1 - \hat{F}_n(t)$$
...
To express $p_j$ more formally,
$$p_j = P(T > t_j| T > t_{j-1}) = \dfrac{P(T > t_j, T > t_{j-1})}{P(T > t_{j-1})} = \dfrac{P(T > t_j)}{P(T > t_{j-1})}$$
Recall that the survival function is defined as
$$S(t) = P(T > t)$$

Hence, rewriting $p_j$ results in
$$p_j = \dfrac{S(t_j)}{S(t_{j-1})} \tag{1}$$

We observe a recursive relationship
$$S(t_0) = S(0) = 1$$
$$p_1 = \dfrac{S(t_1)}{S(t_0)} = S(t_1)$$
$$p_2 = \dfrac{S(t_2)}{S(t_1)}$$
$$p_2 = \dfrac{S(t_3)}{S(t_2)}$$
$$...$$
$$p_k = \dfrac{S(t_k)}{S(t_{k-1})}$$

Hence, continuing from $(1)$
$$p_j = S(t_1) \dfrac{S(t_2)}{S(t_1)} \dfrac{S(t_3)}{S(t_2)} \cdots  \dfrac{S(t_k)}{S(t_{k-1})} = S(t_k)$$
...
We can then estimate the conditional probability of failure at time $t_j$ and the conditional probability of survival as
$$\hat{q}_j = \dfrac{d_j}{n_j}$$
$$\hat{p}_j = 1 - \hat{q}_j = \dfrac{n_j - d_j}{n_j}$$

To estimate the overall survival probability at time $t_k$, we multiply all the conditional probabilities up to that time 
